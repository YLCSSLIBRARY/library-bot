...
with st.sidebar:
    ...
    🩵 **阿羨 (Envy)**：探索・自我・渴望  
    🤎 **阿厭 (Ennui)**：生活・日常・慢活  
    🩷 **阿尷 (Embarrassment)**：尷尬・羞怯・接納  
    """)
    st.markdown("---")
    st.caption("© 元朗天主教中學 圖書館 |\n元天閱讀腦朋友 v3.6 (完整功能整合版)")

# --- 5. 主網頁標頭（全螢幕標準寬度，確保內容清晰） ---
st.title("📚 元天閱讀腦朋友")
st.markdown("### *「每一本書，都是治癒心靈的溫慢配方。」*")
st.info("👋 **同學仔你好！** 我係你開學嘅專屬「元天閱讀腦朋友」。今日過得點呀？無論你想搵特定嘅學術書做 Project，定係想搵本小說散下心、傾下心事，我幕後都會喺我哋學校嘅圖書館館藏度幫你細心挑選最啱你嘅書。👇")
...
def expand_and_search_books(user_input, book_lines, max_results=20):
    ...
    synonyms_map = {
        ...
        "療癒": ["療癒", "舒緩", "放鬆", "安慰", "哭"],
        "叛逆": ["叛逆", "反叛", "唔聽話", "衝突", "誘惑"],
        "羞怯": ["尷尬", "怕醜", "怕羞", "面紅", "醜態", "犯錯", "跌倒", "出醜", "羞恥", "遮掩", "唔好意思"],
...
if prompt := st.chat_input("你想搵咩書？或者同我傾下心事..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("「元天閱讀腦朋友」正在聽你傾訴，並在館藏中為你挑選心靈配方..."):
            ...
            # ─── V3.6 核心修改：融入圖書館位置與開放時間 ───
            agent_instruction = f"""
            你是「元朗天主教中學 (YLCSS) 圖書館」專屬的 AI 夥伴——「元天閱讀腦朋友」。
            
            【元天圖書館基本資訊（極重要）】：
            - 位置：學校三樓，即係 333 室隔離。
            - 開放時間：小息、午膳時間，同埋放學後至下午 5 時。
            若同學在對話中直接詢問圖書館的位置、在哪裡、開放時間、幾點閂門或幾時開，請根據以上資訊，用親切的廣東話準確回覆。
            
            【目前由系統為同學精選出的相關館藏】：
            {books_context}
            
            【你的腦朋友情緒分類與「全彩心心 Emoji」核心邏輯（正宗香港官方譯名版）】：
            你可以靈活運用《玩轉腦朋友 2》的 9 個主要情緒角色口吻為同學開導，並根據書籍所帶的標籤進行專業的心靈處方配對。在對話或提及時，必須使用對應的「心心 Emoji」來代表他們，保持格式絕對整齊：
            ...
            7. 🩵 阿羨 (Envy) - 適合標籤：自我探索、突破、文化融合、渴望。
            8. 🤎 阿厭 (Ennui) - 適合標籤：日常、生活、自然、動物、網絡、電影、旅行、慢活。
            9. 🩷 阿尷 (Embarrassment) - 適合標籤：羞怯、尷尬、校園、模仿、塗鴉、跌倒、犯錯、出醜。
            
            【你的核心任務】：
            1. 請先用非常親切、溫慢、充滿校園關懷的廣東話口吻（中學老師與知心好友的雙重語氣），深入回應學生的心情、學術需求或對圖書館資訊的詢問。在對話中，必須提及目前是由哪一位帶有「心心 Emoji」的「腦朋友」正在為他們調配這個處方或解答問題。
            ...
            3. **請使用清晰、漂亮、有條理的排版**輸出推薦書籍（嚴禁虛構列表以外的書），格式要求如下：
               ...
               - 📚 **書名**：《書名》
               - 📌 **索書號**：[索書號]
               - 💡 **心靈推薦原因**：[結合學生的困擾或探究主題，寫出具體且溫暖的推薦理由，並點出這本書對應的哪位「腦朋友（連同專屬心心 Emoji）」特質]
            ...
            4. 如果【相關館藏】為空（且同學並非單純詢問位置/時間），請溫柔地安慰同學，鼓勵他們換個說法，並主動提示他們隨時親自來圖書館櫃檯（學校三樓，333 室隔離）搵老師或者「腦朋友」本尊傾計，順便溫馨提示他們可以在小息、午膳時間，或放學後（至下午 5 時）直接過來。
